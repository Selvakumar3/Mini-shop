...
function ExporttoPdf(id, filename,contentlist = []) {
    ...
    // var newdat = "Date / Time Exported : "+ today;
    for (let i = 0; i < contentlist.length; i++) {
        doc.text(40, y = y + 10, contentlist[i]).setFontSize(10);
        y=y+7
    }
    ...
    // // Load the image as a base64 data URL
    // img.src = imgURL;

    // img.onload = function () {
        // Once the image is loaded, use it in the PDF
        // doc.addImage(imgurl,'JPG',40,30,80,60);

        // Save the PDF with the specified filename
        // doc.save(filename);
    
    // doc.text(50, y = y + 20, "SALARY STATEMENT FOR THE MONTH OF (VDA AS PER 01.04.2023)").setFontSize(8);
    // doc.text(50, y = y + 30, newdat);  
    doc.autoTable({
        html: '#' + id,
        startY: y,
        styles: {
    ...
    // doc.setFontSize(12);
    // doc.text('Page No: ' + pageCurrent + '/' + pageCount, 20, 20);
    // }
...
function removeHtmlTags(inputString) {
    ...
    let textOnly = inputString.replace(/<[^>]*>/g, "");
...
function ExporttabletoPdfmain(id,filename,colcount,contentlist = []) {
    ...
    doc.setLineWidth(2);
    for (let i = 0; i < contentlist.length; i++) {
        doc.text(40, y = y + 10, contentlist[i]).setFontSize(12);
        y = y + 7;
    }
    ...
    // var columns = [
    //     { title: "Column 1", dataKey: "col1" },
    //     { title: "Column 2", dataKey: "col2" },
    //     { title: "Column 3", dataKey: "col3" },
    //     { title: "Column 4", dataKey: "col4" }
    // ];
    // Prepare the data for the table header
    // var headerData = [];

    // // Iterate through the table rows and extract data for the table header from the first 4 rows
    var columns = []
    var rows = table.getElementsByTagName('tr');
    var headerrow = rows[0].getElementsByTagName('th');
    for (var i = 0; i < colcount ; i++) { // Include the first 4 rows as the table header
        var cells = headerrow[i]
        var rowData = {title: cells.textContent, dataKey: `col${i}`,halign: 'left'};
        // rowData.col1 = cells.textContent;
        // rowData.col2 = cells.textContent;
        // rowData.col3 = cells.textContent;
        // rowData.col4 = cells.textContent;
        // console.log(rowData)
    ...
    // Iterate through the table rows and extract data for the specified columns for the first 4 rows
    var rows = table.getElementsByTagName('tr');
    for (var i = 1; i <= rows.length-1 ; i++) { // Include only the first 4 rows
        var cells = rows[i].getElementsByTagName('td');
        var rowData = {};
        for (var j = 0; j < colcount ; j++) {
            if (cells[j]){
                // console.log(cells[j])
                // rowData[`col${j}`] =cells[j].innerHTML.replaceAll(' ','');
                
                rowData[`col${j}`] = removeHtmlTags(cells[j].innerHTML);
                
            }
            // rowData[`col${j}`] ={ text: cells[j].textContent, halign: 'left' }
        }
        data.push(rowData);
    }
